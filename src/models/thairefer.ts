...
export class ThaiReferModel {
  ...
  person(db: Knex, hospcode, columnSearch, valueSearch, pages = 0) {
    let sql = db("referout")
      .select("hcode as HOSPCODE")
      .select(
        "hn as PID",
        "cid as CID",
        "pname as PRENAME",
        "fname as NAME",
        "lname as LNAME",
        "savedbdate as D_UPDATE"
      )
      .select(
        db.raw(
          `case when sex='ชาย' then '1' when sex='หญิง' then '2' else '0' end as SEX`
        )
      )
      .where("hcode", hospcode)
      .where(columnSearch, valueSearch)
      .offset(pages * maxLimit)
      .limit(maxLimit);
    return sql;
    //return db("referout").where(columnSearch, valueSearch).limit(maxLimit);
    //    return db("referout").limit(10);
  }

  referOut(db: Knex, columnSearch, valueSearch, pages = 0) {
    let sql = db("referout")
      .select("hcode as HOSPCODE")
      .select("refer_no as REFERID_PROVINCE")
      .select(db.raw("case when refer_no_his='' then refer_no else refer_no_his end as REFERID"))
      .select(
        "hn as PID",
        "vn as SEQ",
        "cid as CID",
        "refer_date as DATETIME_SERV",
        "refer_date as DATETIME_REFER",
        "refer_hospcode as HOSP_DESTINATION",
        "cc as CHIEFCOMP",
        "memodiag as DIAGLAST",
        "ClinicGroup as CLINIC_REFER",
        "cause_referout_id as CAUSEOUT",
        "doctor_id as PROVIDER"
      )
      .select(db.raw(`1 as refer_provider`))
      .select(db.raw(`'Thai Refer' as his`))
      .select(
        db.raw(
          "case when refertype_id in ('02','03','04','09','10') then 3 else 1 end as referout_type"
        )
      )
      .select(
        db.raw(`case 
        when strength_id='R' then 1 
        when strength_id='E' then 2 
        when strength_id='U' then 3 
        when strength_id='SU' then 4 
  ...
  sumReferOutByDate(db: Knex, columnSearch, valueSearch) {
    let sql = db("referout")
      .select(db.raw(`date('refer_date') as referDate`))
      .count("hn as cases")
      .where(columnSearch, valueSearch)
      .groupBy(`referout.refer_date`)
      .limit(maxLimit);
    return sql;
  }

  sumReferOutByHCode(db: Knex, columnSearch, valueSearch) {
    let sql = db("referout")
      .select(`hcode`)
      .count("hn as cases")
      .where(columnSearch, valueSearch)
  ...
  address(db: Knex, hospcode, columnSearch, valueSearch) {
    let sql = db("referout")
      .select("hcode as HOSPCODE")
      .select("hn as PID")
      .select(db.raw(`'2' as ADDRESSTYPE`))
      .select(db.raw(`'9' as HOUSETYPE`))
      .select(
        "addrpart as HOUSENO",
        "moopart as VILLAGE",
        "chwpart as CHANGWAT",
  ...
  service(db: Knex, columnSearch, valueSearch, pages = 0) {
    return db("service")
      .where(columnSearch, valueSearch)
      .offset(pages * maxLimit)
      .limit(maxLimit);
  ...
  opdDx(db: Knex, hcode, refer_no) {
    const sql = db("referout_diag as dx")
      .select(
        "dx.hcode as HOSPCODE",
        "dx.refer_no as REFERID",
        "dx.icd10 as DIAGCODE",
        "dx.diagtype_id as DIAGTYPE"
      )
      .where({ hcode, refer_no })
      .limit(maxLimit);
    return sql;
  }

  opdDrug(db: Knex, columnSearch, valueSearch) {
    return db("drug_opd").where(columnSearch, valueSearch).limit(maxLimit);
  }

  admission(db: Knex, columnSearch, valueSearch) {
    return db("admission").where(columnSearch, valueSearch).limit(maxLimit);
  }

  ipdDx(db: Knex, columnSearch, valueSearch) {
    return db("diagnosis_ipd").where(columnSearch, valueSearch).limit(maxLimit);
  }

  ipdDrug(db: Knex, columnSearch, valueSearch) {
    return db("drug_ipd").where(columnSearch, valueSearch).limit(maxLimit);
  }

  labResult(db: Knex, columnSearch, valueSearch) {
    return db("lab_result").where(columnSearch, valueSearch).limit(maxLimit);
  }

  xrayResult(db: Knex, columnSearch, valueSearch) {
    return db("xray_result").where(columnSearch, valueSearch).limit(maxLimit);
  }

  careRefer(db: Knex, columnSearch, valueSearch) {
    return db("care_refer").where(columnSearch, valueSearch).limit(maxLimit);
  }

  referResult(db: Knex, columnSearch, valueSearch) {
    return db("refer_result").where(columnSearch, valueSearch).limit(maxLimit);
  }
  ...
  getData(
    ...
    let sql = db(tableName)
      .select(tableName + ".*")
      .limit(maxLimit);
    if (columnSearch && valueSearch) sql.where(columnSearch, valueSearch);
